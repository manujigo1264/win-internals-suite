...
static void parse_imports(BYTE* base, IMAGE_NT_HEADERS* nt, PEInfo& out) {
    auto& dir = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT];
    if (!dir.VirtualAddress || !dir.Size) return;
    auto imp = (IMAGE_IMPORT_DESCRIPTOR*)rva_to_ptr(base, nt, dir.VirtualAddress);
    if (!imp) return;
    for (; imp->Name; ++imp) {
        auto dll = (char*)rva_to_ptr(base, nt, imp->Name); if (!dll) continue;
        auto thunk = (IMAGE_THUNK_DATA*)rva_to_ptr(base, nt, imp->OriginalFirstThunk ? imp->OriginalFirstThunk : imp->FirstThunk);
        for (; thunk && thunk->u1.AddressOfData; ++thunk) {
            if (thunk->u1.Ordinal & IMAGE_ORDINAL_FLAG) {
                std::ostringstream os; os << dll << "!#" << (thunk->u1.Ordinal & 0xFFFF);
                out.imports.push_back(os.str());
            }
            else {
                auto ibn = (IMAGE_IMPORT_BY_NAME*)rva_to_ptr(base, nt, (DWORD)thunk->u1.AddressOfData);
                if (!ibn) continue;
                std::ostringstream os; os << dll << "!" << (char*)ibn->Name;
                out.imports.push_back(os.str());
            }
        }
    }
...
static void parse_exports(BYTE* base, IMAGE_NT_HEADERS* nt, PEInfo& out){
    auto& dir = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    if (!dir.VirtualAddress || !dir.Size) return;
    auto exp = (IMAGE_EXPORT_DIRECTORY*)rva_to_ptr(base, nt, dir.VirtualAddress);
    if (!exp) return;
    auto names = (DWORD*)rva_to_ptr(base, nt, exp->AddressOfNames);
    auto ords  = (WORD*) rva_to_ptr(base, nt, exp->AddressOfNameOrdinals);
    auto funcs = (DWORD*)rva_to_ptr(base, nt, exp->AddressOfFunctions);
    if (!funcs) return;

    if (names && ords && exp->NumberOfNames){
        for (DWORD i=0; i<exp->NumberOfNames; ++i){
            auto nm = (char*)rva_to_ptr(base, nt, names[i]);
            WORD ord = ords[i] + exp->Base;
            DWORD rva = funcs[ords[i]];
            std::ostringstream os; os << (nm?nm:"<noname>") << " @" << ord << " rva=0x" << std::hex << rva;
        ...
        for (DWORD i=0; i<exp->NumberOfFunctions; ++i){
            ...
            DWORD rva = funcs[i];
            std::ostringstream os; os << "<noname> @" << ord << " rva=0x" << std::hex << rva;
            out.exports.push_back(os.str());
        }
    }
}
...
bool parse_pe_file(const wstring& path, PEInfo& out){
    MappedFile mf; if (!mf.open(path)) return false;
    auto base = mf.base; if (!base) return false;
    ...
    parse_imports(base, nt, out);
    parse_exports(base, nt, out);

    string h; if (sha256(base, mf.size, h)) out.sha256 = h;
    return true;
}
